%% animate_robot.m
%
% Description:
%   Animates the robot according to a list of configurations over time.
%   
% Inputs:
%   q_list: 3xN list of configurations q, where q = [theta_1; theta_2; theta_3];
%   params: a struct with many elements, generated by calling init_params.m
%   varargin: optional name-value pair arguments:
%       'trace_cart_com': (default: false), if true, plots a tracer on the
%           cart's center of mass (CoM)
%       'trace_pend_com': (default: false), if true, plots a tracer on the
%           pendulum's center of mass (CoM)
%       'trace_pend_tip': (default: false), if true, plots a tracer on the
%           pendulum's tip
%       'video': (default: false), if true, animation is written to file.
%
% Outputs:
%   none

function animate_robot(q_list,params,varargin)

% Parse input arguments
% Note: a simple robot animation function doesn't need this, but I want to
% write extensible code, so I'm using "varargin" which requires input
% parsing. See the reference below:
%
% https://people.umass.edu/whopper/posts/better-matlab-functions-with-the-inputparser-class/

% Step 1: instantiate an inputParser:
p = inputParser;

% Step 2: create the parsing schema:
%   2a: required inputs:
addRequired(p,'triple_pend_config', ...
    @(q_list) isnumeric(q_list) && size(q_list,1)==3);
addRequired(p,'triple_pend_params', ...
    @(params) ~isempty(params));
%   2b: optional inputs:
%       optional name-value pairs to trace different parts of the robot:
addParameter(p, 'trace_top_com', false);
addParameter(p, 'trace_top_tip', false);
addParameter(p, 'trace_mid_com', false);
addParameter(p, 'trace_mid_tip', false);
addParameter(p, 'trace_bot_com', false);
addParameter(p, 'trace_bot_tip', false);
addParameter(p, 'video', false);


% Step 3: parse the inputs:
parse(p, q_list, params, varargin{:});

fig_handle = figure('Renderer', 'painters', 'Position', [10 10 900 600]);

if (p.Results.trace_top_com || p.Results.trace_top_tip ...
        || p.Results.trace_mid_com || p.Results.trace_mid_tip ...
        || p.Results.trace_bot_com || p.Results.trace_bot_tip)
    tracing = true;
else
    tracing = false;
end
    
    if p.Results.video
        v = VideoWriter('swingup.avi');
        open(v);
    end
    
    if tracing
        top.curr.com.x = [];
        top.curr.com.y = [];
        
        top.curr.tip.x = [];
        top.curr.tip.y = [];
        
        mid.curr.com.x = [];
        mid.curr.com.y = [];
        
        mid.curr.tip.x = [];
        mid.curr.tip.y = [];
        
        bot.curr.com.x = [];
        bot.curr.com.y = [];
        
        bot.curr.tip.x = [];
        bot.curr.tip.y = [];
    end
    
    for i = 1:size(q_list,2)
        plot_robot(q_list(:,i),params,'new_fig',false);
        
        if tracing
            FK = fwd_kin(q_list(:,i),params);

            % append (x,y) location of top link CoM:
            top.curr.com.x = [top.curr.com.x, FK(1,1)];
            top.curr.com.y = [top.curr.com.y, FK(2,1)];
            
            % append (x,y) location of top link tip:
            top.curr.tip.x = [top.curr.tip.x, FK(1,2)];
            top.curr.tip.y = [top.curr.tip.y, FK(2,2)];

            % append (x,y) location of mid link CoM:
            mid.curr.com.x = [mid.curr.com.x, FK(1,3)];
            mid.curr.com.y = [mid.curr.com.y, FK(2,3)];

            % append (x,y) location of mid link tip:
            mid.curr.tip.x = [mid.curr.tip.x, FK(1,4)];
            mid.curr.tip.y = [mid.curr.tip.y, FK(2,4)];
            
            % append (x,y) location of bot link CoM:
            bot.curr.com.x = [bot.curr.com.x, FK(1,5)];
            bot.curr.com.y = [bot.curr.com.y, FK(2,5)];
            
            % append (x,y) location of bot link tip:
            bot.curr.tip.x = [bot.curr.tip.x, FK(1,6)];
            bot.curr.tip.y = [bot.curr.tip.y, FK(2,6)];
            
            if p.Results.trace_top_com
                hold on;
                plot(top.curr.com.x,top.curr.com.y,'o-',...
                    'Color',params.viz.colors.tracers.top_com,...
                    'MarkerSize',3,'LineWidth',2,...
                    'MarkerFaceColor',params.viz.colors.tracers.top_com,...
                    'MarkerEdgeColor',params.viz.colors.tracers.top_com);
                hold off;
            end
            if p.Results.trace_top_tip
                hold on;
                plot(top.curr.tip.x,top.curr.tip.y,'o-',...
                    'Color',params.viz.colors.tracers.top_tip,...
                    'MarkerSize',3,'LineWidth',2,...
                    'MarkerFaceColor',params.viz.colors.tracers.top_tip,...
                    'MarkerEdgeColor',params.viz.colors.tracers.top_tip);
                hold off;
            end
            if p.Results.trace_mid_com
                hold on;
                plot(mid.curr.com.x,mid.curr.com.y,'o-',...
                    'Color',params.viz.colors.tracers.mid_com,...
                    'MarkerSize',3,'LineWidth',2,...
                    'MarkerFaceColor',params.viz.colors.tracers.mid_com,...
                    'MarkerEdgeColor',params.viz.colors.tracers.mid_com);
                hold off;
            end
            if p.Results.trace_mid_tip
                hold on;
                plot(mid.curr.tip.x,mid.curr.tip.y,'o-',...
                    'Color',params.viz.colors.tracers.mid_tip,...
                    'MarkerSize',3,'LineWidth',2,...
                    'MarkerFaceColor',params.viz.colors.tracers.mid_tip,...
                    'MarkerEdgeColor',params.viz.colors.tracers.mid_tip);
                hold off;
            end
            if p.Results.trace_bot_com
                hold on;
                plot(bot.curr.com.x,bot.curr.com.y,'o-',...
                    'Color',params.viz.colors.tracers.bot_com,...
                    'MarkerSize',3,'LineWidth',2,...
                    'MarkerFaceColor',params.viz.colors.tracers.bot_com,...
                    'MarkerEdgeColor',params.viz.colors.tracers.bot_com);
                hold off;
            end
            if p.Results.trace_bot_tip
                hold on;
                plot(bot.curr.tip.x,bot.curr.tip.y,'o-',...
                    'Color',params.viz.colors.tracers.bot_tip,...
                    'MarkerSize',3,'LineWidth',2,...
                    'MarkerFaceColor',params.viz.colors.tracers.bot_tip,...
                    'MarkerEdgeColor',params.viz.colors.tracers.bot_tip);
                hold off;
            end
        end
        
        if p.Results.video
            M(i) = getframe(fig_handle);
            writeVideo(v,M(i));
        end
    end
    
    if p.Results.video
        close(v);
%         movie(gcf,M); % comment this out if you don't want to see replay
    end
end