...
% go to second stage of release if high enough to giant
if (potEner > 40) &  (com_x < 0)
    check2 = 2;
end
    
% release at correct height on right side of bar
if (check2 ==2) && (potEner > -30) && (com_x > 0)
    check = 1;
    check3 = 4;
    dx(1:nq) = q_dot;
...
% if gripper is within bounding box, put constraints back on and center on
% bar by changing x_dot and y_dot
if (xx > 2.7) && (xx < 3.1) && (yy > 1) && (yy < 1.4)
...
if (check3 == 4) && (xx > 2.7) && (xx < 3.1) && (yy > 1) && (yy < 1.4)
    if(xx < 2.9)
        dx(1) = 1;
    elseif(xx > 2.9)
        dx(1) = -1;
